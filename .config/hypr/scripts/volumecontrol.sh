...
function notify_vol
{
    ...
    angle="$(( (($vol+2)/5) * 5 ))"
	#ico="/home/holo/.config/dunst/icons/vol/vol-0.svg"
    ico="${icodir}/vol-${angle}.svg"
    bar=$(seq -s "." $(($vol / 15)) | sed 's/[0-9]//g')
    dunstify "volctl" -a "$vol$bar" "$nsink" -i $ico -r 91190 -t 800
}
...
function notify_mute
{
    ...
    if [ "$mute" == "true" ] ; then
        dunstify "volctl" -a "muted" "$nsink" -i ${icodir}/muted-${dvce}.svg -r 91190 -t 800
    else
        dunstify "volctl" -a "unmuted" "$nsink" -i ${icodir}/unmuted-${dvce}.svg -r 91190 -t 800
    fi
}
